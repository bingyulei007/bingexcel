...
public class TypeAdapterConverter<T> implements ModelAdapter {
	...
	public TypeAdapterConverter(Constructor<T> constructor,
			List<Field> tempConverterFields,
	...
	@Override
	public T unmarshal(ListRow source, FieldMapperHandler fieldHandler) {
		final Object obj;
		try {
	...
	private void setLocalConverter(FieldConverterMapper converterMapper) {
		...
		Class<?> keyFieldType=converterMapper.isPrimitive()?Primitives.wrap(converterMapper.getFieldClass()):converterMapper.getFieldClass();
		FieldValueConverter fieldValueConverter = defaultLocalConverterHandler.getLocalConverter(keyFieldType);
		
		if(fieldValueConverter==null){
			throw new IllegalEntityException(clazz, "can find the converter for fieldType ["+converterMapper.getFieldClass()+"]");
		}
		 ...
		 protected Object initializeValue(Object obj,String value,FieldConverterMapper converterMapper) {
			 ...
			 if(value!=null){
				 if(converterMapper!=null){
					 ...
					 boolean canConvert = converter.canConvert(converterMapper.getFieldClass());
					 if(!canConvert){
						 throw new ConversionException ("the selected converter ["+converter.getClass()+"] cannot handle type ["+converterMapper.getFieldClass()+"]");
					 }
					 
					 Object fieldValue = converter.fromString(value,defaultLocalConverterHandler,converterMapper.getFieldClass());
					 if(fieldValue!=null){
						 try {
